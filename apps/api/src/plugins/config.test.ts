...
describe('config plugin', () => {
  ...
  describe('directory configuration', () => {
    it('should use default directory paths when HOME is set', async () => {
      ...
      process.env.LAKEPIXIE_BASE_DIR = '/test/home/users';

      await app.register(configPlugin);

      expect(app.config.LAKEPIXIE_BASE_DIR).toBe('/test/home/users');
    });

    it('should allow custom LAKEPIXIE_BASE_DIR', async () => {
      process.env.DATABASE_URL = 'postgresql://localhost:5432/test';
      process.env.DATABRICKS_HOST = 'test.databricks.com';
      process.env.LAKEPIXIE_BASE_DIR = '/custom/users';

      await app.register(configPlugin);

      expect(app.config.LAKEPIXIE_BASE_DIR).toBe('/custom/users');
    });
  });
